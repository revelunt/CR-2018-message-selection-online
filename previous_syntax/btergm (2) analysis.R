...
model1 <- btergm(g ~ edges +
                   ...
                   nodecov("efficacy") + ## those with more efficacy would select more and be selected more by others
                   nodefactor("region_origin2"), 
                   verbose = T, R = 1000, ncpus = 12, parallel = "multicore"); summary(model1)
...
model2 <- btergm(update.formula(model1@formula, ~ . + 
                                  ...
                                  verbose = T, R = 1000, ncpus = 4, parallel = "multicore"); summary(model2)


## add motivations and dyadic homophily predictors
policy.pref.sim <- evaludative.criteria.sim <- list()
for (i in 1:3) {
policy.pref.sim[[i]] <- policy.pref.diff[[i]] * -1
evaludative.criteria.sim[[i]] <- evaludative.criteria.diff[[i]] * -1
}
...
model3 <- btergm(update.formula(model2@formula, ~ . +
                                  ...
                                  nodematch("candidate.preference", diff = TRUE) + 
                                  
                                  edgecov(policy.pref.sim) + ## policy preference difference on ED metric. expect negative relationship
                                  edgecov(evaludative.criteria.sim)),
                                  verbose = T, R = 1000, ncpus = 12, parallel = "multicore"); summary(model3)

## here we need to estimate goodness of fit.
model3.gof <- gof(model3, statistics = c(dsp, esp, odeg, ideg, geodesic, rocpr), 
                  nsim = 400, verbose = TRUE, ncpus = 6, parallel = "multicore")
...
model4 <- btergm(update.formula(model3@formula, ~ . + 
                                  ...
                                  ctriple("candidate.preference")),
                                  verbose = T, R = 1000, ncpus = 12, parallel = "multicore"); summary(model4)  
...
final.model <- btergm(update.formula(model4@formula, ~ . 
                                     + odegree(0) + idegree(1) + esp(0)),
                      verbose = T, R = 1000, ncpus = 12, parallel = "multicore"); summary(final.model) 
...
save(model1, model2, model3, model4, final.model, file = "R_data/btergm.results.June 22th.Rdata")
save(model3.gof, model4.gof, final.model.gof, file = "R_data/btergm.gof.results.June 22th.Rdata")
...
ggdat_fig[, y := factor(var)]

ggplot(ggdat_fig, aes(x = coef, y = y, xmin = q025, xmax = q975)) +
