...
g1 <- simplify(g1, remove.multiple = T, remove.loops = T)
...
g3 <- simplify(g3, remove.multiple = T, remove.loops = T)
...
policy.pref.diff <- varDyadCovar(policy.pref.diff, centered = T)
...
## set dependent variables
  ...
  mydata <- RSiena::sienaDataCreate(discussion.net, candidate.preference,
                            age, gender, edu, policy.pref.diff,
                            liberal.issue.stance, conserv.issue.stance, evaludative.criteria.diff, 
  ...
  save(mydata, file = "R_data/RSIENA_test.Rdata")
  
  setwd("/Users/songh95/Dropbox/(17) 2017 Spring/network QAP Korean election")
  load("R_data/RSIENA_test.Rdata")
...
myalgorithm <- sienaAlgorithmCreate(useStdInits = T, projname = "test.June09.txt", 
                                    seed = 12345, diagonalize = 0.2, doubleAveraging = 0)

# add baseline control effects for network evolution: age, gender, edu status, and pol.ideology
myeff <- includeEffects(myeff, RateX, type = "rate", interaction1 = "interest")
myeff <- includeEffects(myeff, RateX, type = "rate", name = "candidate.preference", interaction1 = "interest")
myeff <- includeEffects(myeff, RateX, type = "rate", interaction1 = "edu")
myeff <- includeEffects(myeff, RateX, type = "rate", name = "candidate.preference", interaction1 = "edu")

myeff <- includeEffects(myeff, egoX, interaction1 = "interest")
myeff <- includeEffects(myeff, eogX, altX, interaction1 = "knowledge")
#myeff <- includeEffects(myeff, sameX, interaction1 = "pol.ideology") ## same ideology

# add some more effects
myeff <- includeEffects(myeff, transTrip, cycle3, inPop, outPop, outAct)
...
Model1 <- RSiena::siena07(myalgorithm, data = mydata, effects = myeff, useCluster = T, 
                  nbrNodes = 8, batch = F, clusterType = "FORK", returnDeps = F) 	
