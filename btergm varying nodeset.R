...
for (i in 1:length(date.range)) {
  ...
  g[[i]] <- g[[i]][vids %in% nodelist, vids %in% nodelist]
...
##------------------------------------------##
  ...
  if (i <= 7) {
    g[[i]] %v% "candidate.image.Park" <- dat[vids, pv194:pv208][, (Mean = rowMeans(.SD)), by = vids][vids %in% nodelist,V1]
    g[[i]] %v% "candidate.image.Moon" <- dat[vids, pv209:pv223][, (Mean = rowMeans(.SD)), by = vids][vids %in% nodelist,V1]
    g[[i]] %v% "candidate.preference" <- dat[vids, as.numeric(canpref1)][vids %in% nodelist]
    g[[i]] %v% "liberal.issue.stance" <- apply(dat[vids, c("pv299", "pv303"), with = F], 1, mean)[vids %in% nodelist]
    g[[i]] %v% "conserv.issue.stance" <- apply(dat[vids, c("pv300", "pv304"), with = F], 1, mean)[vids %in% nodelist]
    temp.test <- dat[vids, .(internet.news.use = (60*pv310 + pv311)/60,
                             newspaper.use = (60*pv312 + pv313)/60, 
                             tv.news.use = (60*pv316 + pv317)/60)]
    g[[i]] %v% "media.use.freq" <- apply(temp.test, 1, mean)[vids %in% nodelist]
    g[[i]] %v% "talk.freq" <- dat[vids, as.numeric(pv322)][vids %in% nodelist]
  } else if (i <= 21) {
    g[[i]] %v% "candidate.image.Park" <- dat[vids, kv7:kv21][, (Mean = rowMeans(.SD)), by = vids][vids %in% nodelist,V1]
    g[[i]] %v% "candidate.image.Moon" <- dat[vids, kv22:kv36][, (Mean = rowMeans(.SD)), by = vids][vids %in% nodelist,V1]
    g[[i]] %v% "candidate.preference" <- dat[vids, as.numeric(canpref2)][vids %in% nodelist]
    g[[i]] %v% "liberal.issue.stance" <- apply(dat[vids, c("kv39", "kv43"), with = F], 1, mean)[vids %in% nodelist]
    g[[i]] %v% "conserv.issue.stance" <- apply(dat[vids, c("kv40", "kv44"), with = F], 1, mean)[vids %in% nodelist]
    temp.test <- dat[vids, .(internet.news.use = (60*kv193 + kv194)/60,
                             newspaper.use = (60*kv195 + kv196)/60, 
                             tv.news.use = (60*kv199 + kv200)/60)]
    g[[i]] %v% "media.use.freq" <- apply(temp.test, 1, mean)[vids %in% nodelist]
    g[[i]] %v% "talk.freq" <- dat[vids, as.numeric(kv217)][vids %in% nodelist]
  } else {
    g[[i]] %v% "candidate.image.Park" <- dat[vids, hv32:hv46][, (Mean = rowMeans(.SD)), by = vids][vids %in% nodelist,V1]
    g[[i]] %v% "candidate.image.Moon" <- dat[vids, hv47:hv61][, (Mean = rowMeans(.SD)), by = vids][vids %in% nodelist,V1]
    g[[i]] %v% "candidate.preference" <- dat[vids, as.numeric(canpref3)][vids %in% nodelist]
    g[[i]] %v% "liberal.issue.stance" <- apply(dat[vids, c("hv84", "hv88"), with = F], 1, mean)[vids %in% nodelist]
    g[[i]] %v% "conserv.issue.stance" <- apply(dat[vids, c("hv85", "hv89"), with = F], 1, mean)[vids %in% nodelist]
    temp.test <- dat[vids, .(internet.news.use = (60*hv252 + hv253)/60,
                             newspaper.use = (60*hv254 + hv255)/60, 
                             tv.news.use = (60*hv258 + hv259)/60)]
    g[[i]] %v% "media.use.freq" <- apply(temp.test, 1, mean)[vids %in% nodelist]
    g[[i]] %v% "talk.freq" <- dat[vids, as.numeric(hv276)][vids %in% nodelist]
  }
  
  
  g[[i]] %n% "policy.pref.sim" <- 1 / (1 + sqrt(outer(g[[i]] %v% "liberal.issue.stance", g[[i]] %v% "liberal.issue.stance", "-")^2 + 
                                  outer(g[[i]] %v% "conserv.issue.stance", g[[i]] %v% "conserv.issue.stance", "-")^2))
  
  g[[i]] %v% "criteria.competence" <- dat[vids, c("pv267", "pv268", "pv269", "pv270", 
                                     "hv19", "hv20", "hv21", "hv22"), with = F][ , (Mean = rowMeans(.SD)), by = vids][vids %in% nodelist,V1]
  g[[i]] %v% "criteria.background" <- dat[vids, c("pv271", "pv272", "pv273", "pv274", 
                                     "pv275", "hv23", "hv24", "hv25", "hv26", "hv27"), with = F][ , (Mean = rowMeans(.SD)), by = vids][vids %in% nodelist,V1]
  ...
  g[[i]] %v% "knowledge" <- rowSums(dat[vids, .(KN1 = recode(pv177, "2 = 1; else = 0"),
                                                ...
                                                KN0 = recode(pv185_1, "4 = 1; else = 0"))])[vids %in% nodelist]
  
  g[[i]] %v% "interest" <- dat[vids, pv165:pv166][, rowMeans(.SD), by = vids][vids %in% nodelist,V1] 
  g[[i]] %v% "expertise" <- apply(cbind(scale(g[[i]] %v% "knowledge", scale = T), scale(g[[i]] %v% "interest", scale = T)), 1, mean)
  g[[i]] %v% "internal.efficacy" <- dat[vids, pv126:pv129][, rowMeans(.SD), by = vids][vids %in% nodelist,V1]
  g[[i]] %v% "consistency.motivation" <- apply(consistency.motivation, 1, mean)[vids %in% nodelist]
  g[[i]] %v% "understanding.motivation" <- apply(understanding.motivation, 1, mean)[vids %in% nodelist]
  g[[i]] %v% "hedomic.motivation" <- apply(hedomic.motivation, 1, mean)[vids %in% nodelist]
  g[[i]] %v% "age" <- dat[vids, as.numeric(age)/10][vids %in% nodelist] ## age in ten years
  g[[i]] %v% "gender" <- dat[vids, as.numeric(sex) - 1][vids %in% nodelist] ## 1 = female, 0 = male
  g[[i]] %v% "edu" <- dat[vids, as.numeric(edu)][vids %in% nodelist] ## education level (1 = "less than elemantry" vs. 9 = "more than postgraduate")
  ...
  g[[i]] %v% "region_origin" <- dat[vids, recode(as.numeric(region1), "1 = 1; 
                                                 ...
                                                 5 = 5; 11:12 = 5; 
                                                 6 = 6; 9:10 = 6;
                                                 15 = 7")][vids %in% nodelist]
  ## region of origin2 -- seoul vs all other
  g[[i]] %v% "region_origin2" <- dat[vids, recode(as.numeric(region1), "1 = 1; else = 0")][vids %in% nodelist] 
  ...
  if (i == 1) { 
    ...
    g_pre <- g[[(i-1)]]
    row.sum <- rowSums(as.matrix(g_pre))
    row.sum <- row.sum[match(nodelist, g_pre %v% "vertex.names")]
    row.sum[is.na(row.sum)] <- 0
    col.sum <- rowSums(as.matrix(g_pre))
    col.sum <- col.sum[match(nodelist, g_pre %v% "vertex.names")]
    col.sum[is.na(col.sum)] <- 0
    g[[i]] %v% "lagged.sender.effect" <- row.sum
    g[[i]] %v% "lagged.receiver.effect" <- col.sum
    
    temp <- g_pre[match(nodelist, g_pre %v% "vertex.names"), match(nodelist, g_pre %v% "vertex.names")]
    temp[is.na(temp)] <- 0
    rownames(temp) <- colnames(temp) <- nodelist
...
for (i in 1:length(g)) {
  g[[i]] %n% "evaludative.criteria.sim" <- 1 / (1 + sqrt(outer(g[[i]] %v% "criteria.competence", g[[i]] %v% "criteria.competence", "-")^2 + 
